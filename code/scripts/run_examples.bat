...
call "%SCRIPT_DIRECTORY%run_batch.bat" --problem bpp-p --input "%BASE_INSTANCE%" --time-limit 10 --threads "%THREADS%" --output-dir "%OUTPUT_ROOT%\bpp-p"
if errorlevel 1 exit /b %ERRORLEVEL%
call "%SCRIPT_DIRECTORY%run_batch.bat" --problem salbp-i --input "%REPOSITORY_ROOT%\data\instances\scholl\Bowman\Bowman_c20.txt" --time-limit 10 --threads "%THREADS%" --output-dir "%OUTPUT_ROOT%\salbp-i"
if errorlevel 1 exit /b %ERRORLEVEL%
...
:usage_error
echo Usage: %SCRIPT_NAME% [--threads N] [--output-dir DIR]
exit /b 2
